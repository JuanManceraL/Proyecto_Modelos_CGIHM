...
ChessGame::ChessGame() : pick_place(true) {
    // Inicializaci�n de las variables miembro en el constructor
...
void ChessGame::HandleMouseClick(const glm::vec3& posMouse) {
    if (posMouse.x == 9999.0f) return; // Posici�n fuera del tablero
...
bool ChessGame::HandleMiddleClick(const glm::vec3& posMouse, Camera& camera) {
    ...
    if (inspectMode) {
        ...
        if (board.haveCharacter(cell.first, cell.second)) {
            ...
            // Mover pieza a posici�n de inspecci�n
            ...
            // Mover c�mara a posici�n de inspecci�n
...
//*****************************************Definici�n de la funci�n para renderizar piezas*****************************************
void RenderChess(Shader* shader, glm::mat4 model) {
    model = glm::mat4(1.0f);
...
//*****************************************************************************************************************************
//Funci�n para convertir coordenadas del mouse a coordenadas del mundo relacionadas a las casillas del tablero (plano Y=0.2)
glm::vec3 ScreenToWorld(double xpos, double ypos, glm::mat4 projection, glm::mat4 view, float planeY = 0.2f) {
    ...
    // Calcular intersecci�n con el plano Y = planeY
    ...
    int boardSize = 8;          // Tama�o 8x8 casillas
