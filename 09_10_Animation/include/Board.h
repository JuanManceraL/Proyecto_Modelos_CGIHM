...
* turn --> Turno del jugador: 1 --> Piezas Blancas, 2 --> Piezas Negras
...
* initWhitesBoard() --> Inicializa las piezas blancas en sus casillas
*/
class Board {
public:
    ...
    bool move(float originX, float originZ, float destinationX, float destinationZ) {
        ...
        if (cells[cellDestination.first][cellDestination.second].occupied) {            //Verificas si existe una pieza en esa casilla
            ...
            cells[cellDestination.first][cellDestination.second].character->alive = 0;
        }
    ...
    void initWhitesBoard(std::vector<Character>& whitePieces) {
        ...
        for (int j = 0; j < 8; j++) {
            cells[0][j].character = &whitePieces[j];
            cells[0][j].occupied = true;
        }
        // Asignar zombies a la segunda fila (cells[1][0] a cells[1][7])
        for (int j = 0; j < 8; j++) {
            cells[1][j].character = &whitePieces[8 + j];  // Índices 8 a 15
            cells[1][j].occupied = true;
        }
    }

    void initBlacksBoard(std::vector<Character>& blackPieces) {
        // Verificar que hay suficientes personajes
        if (blackPieces.size() != 16) {
            std::cerr << "Error: Numero de personajes no admitido por el juego.\n";
            return;
        }
        // Asignar personajes principales a la primera fila (cells[7][0] a cells[7][7])
        for (int j = 0; j < 8; j++) {
            cells[7][j].character = &blackPieces[j];
            cells[7][j].occupied = true;
        }
        // Asignar zombies a la segunda fila (cells[6][0] a cells[6][7])
        for (int j = 0; j < 8; j++) {
            cells[6][j].character = &blackPieces[8 + j];  // Índices 8 a 15
    ...
    bool check_move(Character* piece, int cellOriginI, int cellOriginJ, int cellDestinationI, int cellDestinationJ) {
        ...
        if (piece->team != turn) {
            std::cout << "No es el turno del equipo de la pieza seleccionada.\n";
            return false;
        }
...
//Esquina inferior derecha blancas
